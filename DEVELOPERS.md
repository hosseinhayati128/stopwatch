...
- **Win32 interop & command mode**: `RegisterHotKey` for the global leader hotkey (default Win+F2), dedicated open controller hotkey (default Win+Shift+F2), and dedicated show active overlay hotkey (default Win+Shift+F7). Low-level keyboard hook (`WH_KEYBOARD_LL`) active during the 2-second initial command window and subsequent continuation window (default 0.5s, configurable from 0.2s to 2.0s via `CommandChainingTimeoutSeconds` in settings and shortcut dialogs) to intercept, execute, and chain command keys; `SetWindowLong` for click-through, no-activate, and tool-window styles.
...
The application registers a single global leader hotkey (default: **Win+F2**). Pressing the leader enters command mode (2-second initial window). Executing a command keeps command mode active and resets the window to 0.5 seconds, allowing you to chain subsequent commands (e.g. `Win+F2` → `R` → `T` → `Space`). When no command is pressed within 0.5 seconds, or when `Escape` or an unassigned key is pressed, command mode exits and normal input resumes:
